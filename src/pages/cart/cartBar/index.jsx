...
class CartBar extends Component {
    ...
    setSelectAll() {
        ...
        let data = goods.reduce((prev, item) => {
            let goodsItem = { sku_id:item.item_type==="promotion"?item.item.main_item[0].product_sku_id: item.item.product_sku_id, is_checked: !this.props.selectAll }
        ...
        if(this.props.willCollect.size===this.props.cartData.group.length){
            ...
            let arr=this.props.cartData.group.reduce((prev,item)=>{
                if(item.item_type==="promotion"){
                    prev.push(item.item.main_item[0].product_sku_id)
                }else{
